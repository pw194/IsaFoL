...
sepref_register isa_evsids_flush_int isa_evsids_find_next_undef
  evsids_push_literal

lemma isa_evsids_flush_int_alt_def:
\<open>isa_evsids_flush_int = (\<lambda>M vm (to_remove, h). do {
    ...
    (_, vm, h) \<leftarrow> WHILE\<^sub>T\<^bsup>\<lambda>(i, vm', h). i \<le> n\<^esup>
      ...
      (\<lambda>(i, vm, h). do {
         ...
         let L = to_remove!i;
         vm \<leftarrow> evsids_push_literal L vm;
         ASSERT(atoms_hash_del_pre L h);
         RETURN (i+1, vm, atoms_hash_del L h)})
      (0, vm, h);
    RETURN (vm, (emptied_list to_remove, h))
  })\<close>
  unfolding isa_evsids_flush_int_def Let_def
  by auto

sepref_def evsids_flush_int
  is \<open>uncurry2 isa_evsids_flush_int\<close>
  :: \<open>trail_pol_fast_assn\<^sup>k *\<^sub>a evsids_assn2\<^sup>d *\<^sub>a distinct_atoms_assn\<^sup>d  \<rightarrow>\<^sub>a evsids_assn2 \<times>\<^sub>a distinct_atoms_assn \<close>
  unfolding isa_evsids_flush_int_alt_def emptied_list_alt_def
...
definition evsids0_mset_empty where
  \<open>evsids0_mset_empty = (\<lambda>(_, b, _). b= {#})\<close>

definition hp_evsids_empty where
  \<open>hp_evsids_empty =  (\<lambda>(_, _, _, _, _, h). h = None)\<close>

lemma hp_evsids_empty:
  \<open>(RETURN o hp_evsids_empty, RETURN o evsids0_mset_empty) \<in> 
   (((\<langle>\<langle>nat_rel\<rangle>option_rel, \<langle>Id\<rangle>option_rel\<rangle>pairing_heaps_rel)) O
...
proof -
  ...
  show ?thesis
    ...
    by (intro frefI nres_relI)
     (auto simp add: acids_encoded_hmrel_def evsids0_mset_empty_def encoded_hp_prop_def EVSIDS.hmrel_def encoded_hp_prop_list_conc_def hp_evsids_empty_def pairing_heaps_rel_def H
      split: option.splits)
qed


definition evsids_mset_empty :: \<open>_\<close> where
  \<open>evsids_mset_empty x = (evsids_mset x = {#})\<close>

lemma evsids_mset_empty_alt_def:
   \<open>evsids_mset_empty = (\<lambda>(a, b). evsids0_mset_empty a)\<close>
  by (auto intro!: ext simp: evsids_mset_empty_def evsids0_mset_empty_def
    evsids_mset_def)

sepref_def hp_evsids_empty_code
  is \<open>RETURN o  hp_evsids_empty\<close>
  :: \<open>hp_assn\<^sup>k \<rightarrow>\<^sub>a bool1_assn\<close>
  unfolding hp_assn_def hp_evsids_empty_def atom.fold_option
  by sepref

lemmas [fcomp_norm_unfold] = evsids_assn_def[symmetric]

lemmas [sepref_fr_rules] = hp_evsids_empty_code.refine[FCOMP hp_evsids_empty,
  unfolded hr_comp_assoc[symmetric] evsids_assn_def[symmetric] evsids_assn2_def[symmetric]]

sepref_def evsids_mset_empty_code
  is \<open>RETURN o evsids_mset_empty\<close>
  :: \<open>evsids_assn2\<^sup>k \<rightarrow>\<^sub>a bool1_assn\<close>
  unfolding evsids_mset_empty_alt_def evsids_assn2_def
  by sepref  

sepref_def evsids_find_next_undef_impl
  is \<open>uncurry isa_evsids_find_next_undef\<close>
  :: \<open>evsids_assn2\<^sup>d *\<^sub>a trail_pol_fast_assn\<^sup>k \<rightarrow>\<^sub>a atom.option_assn \<times>\<^sub>a evsids_assn2\<close>
  unfolding isa_evsids_find_next_undef_def
    atom.fold_option evsids_mset_empty_def[symmetric]
  by sepref
...
lemma isa_bump_unset_alt_def:
  ...
  hstable \<leftarrow> (if \<not>foc then evsids_tl L hstable else RETURN hstable);
