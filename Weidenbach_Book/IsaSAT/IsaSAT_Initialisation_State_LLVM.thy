...
type_synonym bump_heuristics_init = \<open>((nat,double\<^sub>p)evsids, vmtf_init, bool, nat list \<times> bool list) tuple4\<close>
...
definition heuristic_bump_init_assn :: \<open>bump_heuristics_init \<Rightarrow> bump_heuristics_init_assn \<Rightarrow> _\<close> where
  \<open>heuristic_bump_init_assn = tuple4_assn evsids_assn2 vmtf_init_assn bool1_assn distinct_atoms_assn\<close>
...
global_interpretation Bump_Heur_Init: tuple4_state where
  ...
  apply (rule bottom_init_vmtf_code.refine bottom_focused.refine free_evsids_assn2
    bottom_atms_hash_code.refine free_vmtf_init_assn_assn2 free_focused_assn2
    free_distinct_atoms_assn2 empty_evsids_code.refine free_evsids_assn2')+
...
definition bottom_init_bump :: \<open>bump_heuristics_init\<close> where
  \<open>bottom_init_bump = Bump_Heuristics_Init empty_evsids bottom_init_vmtf False bottom_atms_hash\<close>
