...
definition mop_imp_needs_rescaling
  :: \<open>double\<^sub>p \<Rightarrow> bool nres\<close> where
  \<open>mop_imp_needs_rescaling = (\<lambda>m. RETURN (m > evsids_limit))\<close>

sepref_def mop_imp_needs_rescaling_code
  is \<open>mop_imp_needs_rescaling\<close>
  :: \<open>dpfloat_assn\<^sup>k \<rightarrow>\<^sub>a bool1_assn\<close>
  unfolding mop_imp_needs_rescaling_def evsids_limit_word_def
  by sepref

sepref_def mop_imp_decreases_weights_only_code
  ...
  unfolding mop_imp_decreases_weights_only_def hp_assn_def
