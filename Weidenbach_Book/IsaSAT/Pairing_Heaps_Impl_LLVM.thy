...
lemma mop_prio_change_all_weights_alt_def:
  "EVSIDS.mop_prio_change_all_weights \<equiv>  (\<lambda>(\<A>, b, w). RES {(\<A>, b,w) |w. True})"
  unfolding EVSIDS.mop_prio_change_all_weights_def bind_RES_RETURN_eq
  by auto
...
lemma encoded_hp_prop_list_conc_rescale_weights:
  \<open> encoded_hp_prop_list_conc (ck, (cl, cm, cn, co, cp), cq) (cr, Some y) \<Longrightarrow>
       encoded_hp_prop_list_conc
        (ck, (cl, cm, cn, co, \<lambda>x. map_option (\<lambda>x. \<beta> * x) (cp x)), cq)
        (cr, Some (hp_rescale_weight \<beta> y))\<close>
  by (auto simp: encoded_hp_prop_list_conc_def encoded_hp_prop_def)

lemma mop_hp_decreases_weights_mop_prio_change_all_weights:
  \<open>(uncurry mop_hp_decreases_weights, uncurry (EVSIDS.mop_prio_change_all_weights'))
   \<in> Id \<times>\<^sub>f acids_encoded_hmrel \<rightarrow>\<^sub>f \<langle>acids_encoded_hmrel\<rangle>nres_rel\<close>
  unfolding  mop_hp_decreases_weights_def mop_prio_change_all_weights_alt_def
     mop_hp_needs_rescaling_def uncurry_def EVSIDS.mop_prio_change_all_weights'_def
     mop_hp_decreases_weights_only_def
  apply (intro frefI nres_relI)
  apply refine_vcg
  apply (auto simp: mop_hp_decreases_weights_def EVSIDS.mop_prio_change_all_weights_def
     mop_hp_needs_rescaling_def acids_encoded_hmrel_def intro!: RETURN_SPEC_refine)
  apply (rule_tac x= \<open>\<lambda>x. x1 * dp x\<close> in exI)
  apply (rule_tac b= \<open>(dl, map_option (hp_rescale_weight x1) dm)\<close> in relcomp.relcompI)
   apply (auto simp: mop_hp_decreases_weights_def EVSIDS.mop_prio_change_all_weights_def EVSIDS.hmrel_def
     mop_hp_needs_rescaling_def acids_encoded_hmrel_def score_hp_rescale_weights_hp_node
     intro!: RETURN_SPEC_refine
     intro: encoded_hp_prop_list_conc_rescale_weights EVSIDS.invar_hp_rescale_weight)
...
lemma mop_imp_decreases_weights_mop_hp_decreases_weights2:
  ...
  subgoal for x y
    apply (cases x; cases y; hypsubst)
    unfolding split
...
lemmas [sepref_fr_rules] =
  mop_imp_decrease_weights_code.refine[FCOMP mop_imp_decreases_weights_mop_hp_decreases_weights2,
    ...
    unfolded evsids_assn_def[symmetric]]
